...
function calLength2(x1, y1, x2, y2) {//(x1,y1),(x2,y2)是一个果实和大鱼的坐标
	//用x1-x2的平方+y1-y2的平方就等于一个三角形中斜边的平方
	//也就是两个果实和大鱼的平方
...
function lerpAngle(a, b, t) {//a是目标角度 b是当前角度
	var d = b - a;
...
function lerpDistance(aim, cur, ratio) {//aim是目标值，cur是当前值，ratio是百分比
	var delta = cur - aim;
	return aim + delta * ratio;//delta是目标值和当前值的距离 返回的是按照一定比率接近目标值的一个值
}
